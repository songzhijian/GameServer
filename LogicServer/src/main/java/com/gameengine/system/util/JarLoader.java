...
public final class JarLoader implements IClassLoader {
	...
	protected void createTmpDir() {
		if (this.copyExtensionJar && copyTargetPath == null){
			String tmpDirName = MD5.getInstance().getHash(String.valueOf(System.currentTimeMillis())).substring(0, 8);
	...
	public ClassLoader loadClasses(String[] paths, ClassLoader parentClassLoader) throws BootException {
        ...
        for (int i = 0; i < paths.length; i++) {
            ...
            if (locations.size() == 0) {
                throw new BootException("Unexpected: sf jars were located!");
            }
        }
	...
	protected void addPathURL(ArrayList locations, String jarFilePath) throws MalformedURLException {
