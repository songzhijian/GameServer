...
import com.gameengine.system.net.codec.ProtocCodecUtil;
 ...
 *	不要用在其他地方否则会导致内存溢出
 */
public class PooledHeapByteBufX extends AbstractReferenceCountedByteBuf {
     ...
     */
    protected PooledHeapByteBufX(ByteBufAllocator alloc, int initialCapacity, int maxCapacity) {
        super(maxCapacity);
     ...
     * @param maxCapacity the max capacity of the underlying byte array
     */
    protected PooledHeapByteBufX(ByteBufAllocator alloc, byte[] initialArray, int maxCapacity) {
        super(maxCapacity);
    ...
    public final static ThreadLocal<PooledHeapByteBufX> REF_LOCAL = new ThreadLocal<PooledHeapByteBufX>();
    
    public final static ThreadLocal<CodedOutputStream> OUTPUT_REF_LOCAL = new ThreadLocal<CodedOutputStream>();
    
    public final static int CACHE_BYTES = 20480;
    
    /**
     * 为了减少CodedOutputStream对象的创建和数组的创建
     */
    public static PooledHeapByteBufX wrappedBuffer(int msgType,
			MessageLite msg) throws IOException {
      	int serializedSize = msg.getSerializedSize();
    	int len = serializedSize + ProtocCodecUtil.MSG_TYPE_LENGTH;
    	int limitSize = CACHE_BYTES;
    	PooledHeapByteBufX cache = null;
    	if (len > limitSize) {
    		cache = new PooledHeapByteBufX(UnpooledByteBufAllocator.DEFAULT, new byte[len], len);
    	} else {
    		cache = REF_LOCAL.get();
    		if(cache == null) {
    			cache = new PooledHeapByteBufX(UnpooledByteBufAllocator.DEFAULT, new byte[limitSize], len);
    		} else {
    			REF_LOCAL.set(null);
    			cache.reset(len);
    		}
    	}
		byte[] result = cache.array;
		result[0] = (byte) (msgType >>> 8);
		result[1] = (byte) msgType;
		CodedOutputStream output = CodedOutputStream.newInstance(result, 2, serializedSize);
        msg.writeTo(output);
    	return cache;
    }
	...
	public static ByteBuf wrappedBuffer(byte[] array) {
		...
		PooledHeapByteBufX cache = REF_LOCAL.get();
		if(cache == null) {
			cache = new PooledHeapByteBufX(UnpooledByteBufAllocator.DEFAULT, array,
					array.length);
		} else {
			REF_LOCAL.set(null);
			cache.reset(array, array.length);
		}
		return cache;
	}
	
	public void reset(byte[] initialArray, int maxCapacity) {
		maxCapacity(maxCapacity);
		checkNotNull(initialArray, "initialArray");
	...
	public boolean release() {
		tmpNioBuf = null;
		if (this.array.length > CACHE_BYTES) {
			this.array = null;
		} else {
			REF_LOCAL.set(this);
		}
